...
d) The styled rewrite (default "Slack") needs one of two engines, chosen in Settings (gear icon): "Rewrite on device (Apple Intelligence)" on iPhones with Apple Intelligence (iOS 26), no key, no network; or "AI provider API key": paste this test key (low spending cap, revoked after review): <<< TEST KEY >>>  The provider is detected from the key; the key is kept in the iOS Keychain and sent only to that provider.
